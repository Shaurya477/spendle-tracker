...
- **Rewards.** Policy sends up to 80% of protocol revenue to a buyback contract that TWAPs USDT→PENDLE hourly via Pendle Router V4. That 80% is a policy figure, not an observed flow: USDT `Transfer`s into the buyback contract since the snapshot total $3.99M against a $5.54M 80% share of DefiLlama's Revenue (11 Sep 2026), and for the 7 Apr–11 Aug fee epochs 54.5% of the app's protocol take. The funding lands around each fee epoch's end and matches the hub's per-epoch "Fees Collected" in 9 of 10 closed epochs. Every ~14 days the same contract calls `stake()` on sPENDLE and transfers the minted sPENDLE to Pendle's Merkle distributor, from which holders claim. Rewards are split pro-rata over *active* sPENDLE **including virtual sPENDLE**.
